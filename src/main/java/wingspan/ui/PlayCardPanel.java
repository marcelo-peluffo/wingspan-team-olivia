...
public class PlayCardPanel extends JPanel implements MouseListener, KeyListener{
    ...
    public void paint(Graphics g)
    {
        ...
        if (placingBird)
        {
            g2d.setStroke(new BasicStroke(10));
            g2d.setColor(new Color(19,175,87));
            if (selectedCard.getBirdInfo().getHabitats().contains(Habitat.FOREST) && hasEnoughEggs(Habitat.FOREST, GameState.activePlayer.getTotalEggsAmount()))
            {
                g2d.drawRect(forestPos.getX(), forestPos.getY(), CARD_WIDTH, CARD_HEIGHT);
            }
            if (selectedCard.getBirdInfo().getHabitats().contains(Habitat.GRASSLANDS) && hasEnoughEggs(Habitat.GRASSLANDS, GameState.activePlayer.getTotalEggsAmount()))
            {
                g2d.drawRect(grasslandsPos.getX(), grasslandsPos.getY(), CARD_WIDTH, CARD_HEIGHT);
            }
            if (selectedCard.getBirdInfo().getHabitats().contains(Habitat.WETLANDS) && hasEnoughEggs(Habitat.WETLANDS, GameState.activePlayer.getTotalEggsAmount()))
            {
